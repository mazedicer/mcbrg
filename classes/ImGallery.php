...
//=======================================================================================//
	...
	public function getImageArray(){
		...
		$path = $this->folderpath . '/{*.jpg,*.gif,*.png,*.JPG}';
	...
	public function display( $showit = 1 ){
		$img_template = file_get_contents( '../templates/gallery_image.php' );
		$replace = array( "{original_img}", "{name}", "{thumbnail}", "{item_num}" );
		$markup = '';
		if( !empty( $this->elements ) ){
			foreach( $this->elements as $img ){
				...
				$item_num = substr( $imgname, 0, $end );
				$original_img = substr( $img, 1 );
				$replace_with = array( $original_img, $imgname, $thumb, $item_num );
				$markup .= str_replace( $replace, $replace_with, $img_template );
			}
		}
		if( $showit == 1 ){
			echo $markup;
	...
	public function getImageThumbnail( $src ){
		...
		if( file_exists( $cachepath ) ){
			...
			switch( $type ){//saving the image into memory (for manipulation with GD Library)   
				case 1: $myImage = imagecreatefromgif( $imgSrc );
					break;
				case 2: $myImage = imagecreatefromjpeg( $imgSrc );
					break;
				case 3: $myImage = imagecreatefrompng( $imgSrc );
					break;
			}
	...
	public function cachePicture( $im, $cachepath ){
		...
		if( function_exists( "imagepng" ) ){
			...
			imagejpeg( $im, $cachepath, 0.5 );
		}elseif( function_exists( "imagewbmp" ) ){
			imagewbmp( $im, $cachepath );
		}else{
